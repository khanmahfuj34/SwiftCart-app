...
export default function SignupScreen() {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.loop(
        Animated.sequence([
          Animated.timing(logoAnim, { toValue: -12, duration: 2000, useNativeDriver: true }),
          Animated.timing(logoAnim, { toValue: 0, duration: 2000, useNativeDriver: true })
        ])
      ),
      Animated.loop(
        Animated.sequence([
          Animated.timing(blob1Anim, { toValue: -20, duration: 4000, useNativeDriver: true }),
          Animated.timing(blob1Anim, { toValue: 0, duration: 4000, useNativeDriver: true })
        ])
      ),
      Animated.loop(
        Animated.sequence([
          Animated.timing(blob2Anim, { toValue: 30, duration: 5000, useNativeDriver: true }),
  ...
  const isFormValid = name.length > 0 && email.length > 0 && password.length > 5 && password === confirmPassword;
    ...
    >
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" bounces={false} showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          ...
          <Animated.View pointerEvents="none" style={[styles.blob1, { transform: [{ translateY: blob1Anim }] }]} />
          <Animated.View pointerEvents="none" style={[styles.blob2, { transform: [{ translateX: blob2Anim }] }]} />

          <View pointerEvents="box-none" style={styles.inner}>
          <Animated.View style={[styles.headerContainer, { opacity: fadeAnim, transform: [{ translateY: logoAnim }] }]}>
          ...
          <Animated.View style={[styles.formContainer, { opacity: fadeAnim, transform: [{ translateX: errorShake }] }]}>
            
            {/* Name Input */}
            <Animated.View style={[styles.inputWrapper, nameFocused && styles.inputWrapperFocused, { transform: [{ scale: nameScale }] }]}>
            ...
            {/* Email Input */}
            <Animated.View style={[styles.inputWrapper, emailFocused && styles.inputWrapperFocused, { transform: [{ scale: emailScale }] }]}>
            ...
            {/* Password Input */}
            <Animated.View style={[styles.inputWrapper, passFocused && styles.inputWrapperFocused, { transform: [{ scale: passScale }] }]}>
              <Text style={styles.inputEmoji}>🔒</Text>
              <TextInput 
                style={styles.input}
            ...
            <Animated.View style={[styles.inputWrapper, confirmPassFocused && styles.inputWrapperFocused, { transform: [{ scale: confirmPassScale }] }]}>
              <Text style={styles.inputEmoji}>🛡️</Text>
              <TextInput 
                style={styles.input}
                ...
                onFocus={() => handleFocus(setConfirmPassFocused, confirmPassScale)}
                onBlur={() => handleBlur(setConfirmPassFocused, confirmPassScale)}
              />
            ...
            >
              <Animated.View style={[styles.loginButtonWrapper, { transform: [{ scale: btnScale }] }]}>
                {isFormValid ? (
                  <LinearGradient
                  ...
                  <View style={[styles.loginButtonGradient, styles.loginButtonDisabled]}>
                    <Text style={styles.loginButtonDisabledText}>CREATE ACCOUNT</Text>
                  </View>
                )}
              </Animated.View>
            </TouchableOpacity>
